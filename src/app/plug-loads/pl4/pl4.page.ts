...
@Component({
  selector: 'app-pl4',
  templateUrl: './pl4.page.html',
  styleUrls: ['./pl4.page.scss'],
})
export class Pl4Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('plug-loads').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().pl4;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('plug-loads').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().pl4);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
