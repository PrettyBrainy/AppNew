...
@Component({
  selector: 'app-pl2',
  templateUrl: './pl2.page.html',
  styleUrls: ['./pl2.page.scss'],
})
export class Pl2Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('plug-loads').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().pl2;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('plug-loads').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().pl2);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
