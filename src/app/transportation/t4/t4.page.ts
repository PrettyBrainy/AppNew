...
@Component({
  selector: 'app-t4',
  templateUrl: './t4.page.html',
  styleUrls: ['./t4.page.scss'],
})
export class T4Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('transportation').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().t4;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('transportation').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().t4);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
