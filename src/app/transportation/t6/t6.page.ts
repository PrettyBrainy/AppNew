...
@Component({
  selector: 'app-t6',
  templateUrl: './t6.page.html',
  styleUrls: ['./t6.page.scss'],
})
export class T6Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('transportation').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().t6;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('transportation').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().t6);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
