...
@Component({
  selector: 'app-t3',
  templateUrl: './t3.page.html',
  styleUrls: ['./t3.page.scss'],
})
export class T3Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('transportation').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().t3;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('transportation').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().t3);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
