...
@Component({
  selector: 'app-t7',
  templateUrl: './t7.page.html',
  styleUrls: ['./t7.page.scss'],
})
export class T7Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('transportation').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().t7;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('computers').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().t7);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
