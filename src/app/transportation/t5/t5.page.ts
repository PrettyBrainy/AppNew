...
@Component({
  selector: 'app-t5',
  templateUrl: './t5.page.html',
  styleUrls: ['./t5.page.scss'],
})
export class T5Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('transportation').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().t5;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('transportation').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().t5);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
