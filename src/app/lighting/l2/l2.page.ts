...
@Component({
  selector: 'app-l2',
  templateUrl: './l2.page.html',
  styleUrls: ['./l2.page.scss'],
})
export class L2Page implements OnInit {
...
verifyPledge(verification){
  ...
  let verificationUpdate = {
    l2: this.verification
  }

  //__________________________________________Update Database with verification
  const verify = firebase.firestore().collection('userProfile').doc(`${uid}`).collection("pledges").doc("lighting").update(verificationUpdate);
  })
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('lighting').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().l2;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('lighting').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().l2);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
