...
verifyPledge(verification){
  ...
  let verificationUpdate = {
    c3: this.verification
  }

  //__________________________________________Update Database with verification
  const verify = firebase.firestore().collection('userProfile').doc(`${uid}`).collection("pledges").doc("computers").update(verificationUpdate);
  })
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('computers').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().c3;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('computers').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().c3);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
