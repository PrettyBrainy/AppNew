...
@Component({
  selector: 'app-ar4',
  templateUrl: './ar4.page.html',
  styleUrls: ['./ar4.page.scss'],
})
export class Ar4Page implements OnInit {
...
verifyPledge(verification){
  ...
  let verificationUpdate = {
    ar4: this.verification
  }

  //__________________________________________Update Database with verification
  const verify = firebase.firestore().collection('userProfile').doc(`${uid}`).collection("pledges").doc("appliancesAndRecycling").update(verificationUpdate);
  })
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('appliancesAndRecycling').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().ar4;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('appliancesAndRecycling').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().ar4);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
