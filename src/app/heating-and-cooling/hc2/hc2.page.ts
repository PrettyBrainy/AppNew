...
@Component({
  selector: 'app-hc2',
  templateUrl: './hc2.page.html',
  styleUrls: ['./hc2.page.scss'],
})
export class Hc2Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('heatingAndCooling').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().hc2;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('heatingAndCooling').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().hc2);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
