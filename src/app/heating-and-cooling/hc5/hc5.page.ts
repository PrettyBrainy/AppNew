...
@Component({
  selector: 'app-hc5',
  templateUrl: './hc5.page.html',
  styleUrls: ['./hc5.page.scss'],
})
export class Hc5Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('heatingAndCooling').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().hc5;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('heatingAndCooling').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().hc5);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
