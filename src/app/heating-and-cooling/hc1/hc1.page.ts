...
@Component({
  selector: 'app-hc1',
  templateUrl: './hc1.page.html',
  styleUrls: ['./hc1.page.scss'],
})
export class Hc1Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('heatingAndCooling').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().hc1;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('heatingAndCooling').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().hc1);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
