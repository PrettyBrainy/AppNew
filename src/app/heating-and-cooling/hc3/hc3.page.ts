...
@Component({
  selector: 'app-hc3',
  templateUrl: './hc3.page.html',
  styleUrls: ['./hc3.page.scss'],
})
export class Hc3Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('heatingAndCooling').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().hc3;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('heatingAndCooling').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().hc3);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
