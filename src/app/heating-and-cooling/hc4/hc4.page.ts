...
@Component({
  selector: 'app-hc4',
  templateUrl: './hc4.page.html',
  styleUrls: ['./hc4.page.scss'],
})
export class Hc4Page implements OnInit {
...
let verfCheck = firebase.firestore().collection('userProfile').doc(`${this.uid}`).collection('pledges').doc('heatingAndCooling').get().then((docSnapshot)=>{
  this.pledgeContent = docSnapshot.data().hc4;
...
checkForPledgeStatus(){
  ...
  .collection('approval').doc('heatingAndCooling').get().then((docSnapshot)=>{
    let status = String(docSnapshot.data().hc4);
    console.log(status);
    if(status == 'pending'){
      console.log(status);
      this.hideVerfCard = true;
